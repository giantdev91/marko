...
const SocialComponent = ({type,close_kind,closeActivity,title,comment}) => {

    const [active, setActive] = useState(true);  
    let icon;
    
    switch(type){
        case "Discord" : icon = Discord_Icon; break;
        case "Twitter" : icon = Twitter_Icon; break;
        case "Youtube" : icon = Youtube_Icon; break;
        case "Medium" : icon = Medium_Icon; break;
        case "Telegram" : icon = Telegram_Icon; break;
        default: break;
    }
            ...
            </div>
            {
                ...
                <div className="ipt-box">
                    <div className='label-row'>
                        <label htmlFor="title1" className="title">URL</label>
                    </div>
                        <InputText id="title1" aria-describedby="username1-help" placeholder={comment} className="ipt-txt"/>
                </div>
                </>:<></>
            }   
        </div>
