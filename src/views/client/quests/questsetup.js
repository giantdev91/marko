...
const QuestSetup = ({setTab}) => {
	...
	const nextTab = () => {
		...
		if(startDate == "" | endDate == "") { setDateEmpty(true);  } else { setDateEmpty(false)}
		
		if(images.length == 0){ setPhotoEmpty(true);} else { setPhotoEmpty(false)}
		if(!titleEmpty && !descriptionEmpty && !dateEmpty && !photoEmpty)
			setTab(2);
		else
			return;
	}
					...
					<div className="admin-setup-component">
						<div className="admin-setup-component-container">
					    	...
					    	<div className="ipt-text-div">
								...
								{ titleEmpty ? <span className='ipt_title_comment'>Please,fill title</span> : <></>}
								</div>
								...
								<div className={`${ descriptionEmpty ? 'description-empty' : ''}`}>
								  ...
								  { descriptionEmpty ? <span className='ipt_title_comment'>Please,fill description</span> : <></>}
							</div>
						</div>
					</div>
					<div className="admin-setup-component">
					    <div style={{'height':'100%'}}>
							...
							{ images.length > 0  ?									
									<Dropzone onDrop={onDrop} accept={"image/*"} isEmpty={ false } photoEmpty={photoEmpty} img={images[0]} deleteImg={deleteImg}/>
									: 
									<Dropzone onDrop={onDrop} accept={"image/*"} isEmpty={ true }  photoEmpty={photoEmpty} img={null} deleteImg={deleteImg}/>
							}
						</div>						
					</div>
				</div>
						...
						<div className="admin-setup-component-container">
							...
							<div className="flex flex-row justify-content-between date-align-horizontal gap-3">
								<div className="flex-grow-1" >
									<QuestCalendar start={true} value={startDate} onChange={setStartDate} empty={ dateEmpty }/>
									
									{ dateEmpty ? <div><span className='ipt_title_comment'>Please pick dates</span></div> : <></>}
								</div>
								<div className="flex-grow-1">
									<QuestCalendar  start={false} value={startDate} onChange={setEndDate} empty={ dateEmpty }/>
									<div className="flex flex-row-reverse mt-3">
										<div className="flex align-items-center quest-setup-date-noending-btn w-full">
											<div className='flex align-items-center'>
													...
													<InputSwitch checked={noedate} onChange={(e) => handleEDate(e.value)} style={{'width':'2.54rem','height':'1.5rem'}}/>
											</div>
